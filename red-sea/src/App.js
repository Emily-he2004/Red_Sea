...
const App = () => {
      ...
      .then(data => {
        ...
        initMap.on('load', () => {
          fetch('/safest-route')
            ...
            .catch(error => console.error('Error fetching the safest route:', error));
        });
      });
  ...
  useEffect(() => {
    if (map && selectedCamp) {
      ...
      fetch(`/safest-route?start_lat=${searchLocation.lat}&start_long=${searchLocation.lng}&end_lat=${campLat}&end_long=${campLng}`)
        .then(response => response.json())
        .then(data => {
          if(data.geometry) {
            const routeGeometry = data.geometry;
  ...
  const handleSearchChange = (e) => {
    ...
    if (searchText.length > 3) { // To avoid too many requests, only search if the query is longer than 3 characters
  ...
  return (
    <div>
      ...
      />
      <select onChange={(e) => setSelectedCamp(camps[parseInt(e.target.value, 10)])}>
      ...
      <div id="map" style={{ position: 'absolute', top: 35, bottom: 0, right: 1, width: '100%' }}></div>
    </div>
  );
};
